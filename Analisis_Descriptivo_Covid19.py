...
"""As we saw, the last 4 columns, wich are the ones related to vaccinations, 
   ...
   infections happened. In addition to that, it may appear that some of the last records
   have missing values beacuse, at the time of downloding the data, the data had not been
   entered into the field.
   
   So, let's filter the data from December 2020 to May 2022 and interpolate the data on the 
   last records."""

covid19us = covid19us.query("date >= '2020-12-14 00:00:00'")

covid19us["icu_patients"] = covid19us["icu_patients"].fillna(
    covid19us["icu_patients"].interpolate())

covid19us["hosp_patients"] = covid19us["hosp_patients"].fillna(
    covid19us["hosp_patients"].interpolate())

covid19us["positive_rate"] = covid19us["positive_rate"].fillna(
    covid19us["positive_rate"].interpolate())

covid19us["total_vaccinations"] = covid19us["total_vaccinations"].fillna(
    covid19us["total_vaccinations"].interpolate())

covid19us["people_vaccinated"] = covid19us["people_vaccinated"].fillna(
    covid19us["people_vaccinated"].interpolate())

covid19us["people_fully_vaccinated"] = covid19us["people_fully_vaccinated"].fillna(
    covid19us["people_fully_vaccinated"].interpolate())

covid19us["new_vaccinations"] = covid19us["new_vaccinations"].fillna(
    covid19us["new_vaccinations"].interpolate())
   
######################### CHANGING THE DATA TYPE OF SOME VARIABLES #########################

covid19us.dtypes

covid19us = covid19us.astype({"icu_patients": int, "hosp_patients": int, "positive_rate":float, 
                          "total_vaccinations":int, "people_vaccinated":int,
                          "people_fully_vaccinated":int, "new_vaccinations":int},
                         errors = "raise")
covid19us["date"] = pd.to_datetime(covid19us["date"])
